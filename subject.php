...
if (isset($_POST['check']) && isset($_POST['session']) &&isset($_POST['term']) && isset($_POST['class']) && isset($_POST['subject'])) {
	...
	$subjectStats = $query->subjectStats($_POST['class'], $_POST['session'], $_POST['term'], $_POST['subject']);
	if ($subjectStats != null) {
		extract($subjectStats);
		$query->updateQuery("scores", ["classavg"=>$sbavg, "min"=>$sbmin, "maxi"=>$sbmax], ["class"=>$_POST['class'], "term"=>$_POST['term'], "subj"=>$_POST['subject'], "session"=>$_POST['session']]);
	}
	

    
    $students = $query->checkSubject($_POST['class'], $_POST['session'], $_POST['term'], $_POST['subject']);
