...
class QueryHandle
{
	...
	{
		$i = $k = 0;
		$length = count($updateCols);
		$length2 = count($searchCols);
	...
	{
		$search = "%$search%";
		$stmt = $this->conn->prepare("SELECT name, admno, 
            MAX(class) AS class, 
            MAX(dater) AS dater, 
            MAX(session) AS session
        FROM students
        WHERE name LIKE :s OR admno LIKE :s
        GROUP BY admno, name
    ");
