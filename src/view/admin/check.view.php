		...
		<div class="container-login100">
			<div class="wrap-login100">
				...
				<form class="login100-form" method="post">
					...
					<?php if (isset($error['cd'])) {
						echo "<span class='text-warning'>" . $error['cd'] . "</span><br>";
					}
					if (isset($error['admn'])) {
						echo "<span class='text-warning'>" . $error['admn'] . "</span><br>";
					}

					if (isset($msg['status'])) {
						echo "<span class='text-warning'>" . $msg['status'] . "</span>";
					}
					?>
					...
					<div class="wrap-input100 validate-input">
						<select name="session" size="1" required aria-required="true" class="input100">
							...
							<?php for ($py = date('Y') - 2; $py <= date('Y') - 1; $py++) {
								$k = $py + 1; ?>
