...
<div class="container-fluid">
                ...
                <h4 class="card-header text-white text-center">SET NUMBER OF SUBJECTS FOR A CLASS IN A TERM</h4>
        ...
        <?php  if (isset($error['class'])) {  echo "<span class='text-danger'>".$error['class']."</span>";  } ?>
         ...
         <?php  if (isset($error['term'])) {  echo "<span class='text-danger'>".$error['term']."</span>";  } ?>    
                    ...
                    <?php  if (isset($error['tsubjects'])) {  echo "<span class='text-danger'>".$error['tsubjects']."</span>";  } ?></div>
                                              </div>
...
<button type="submit" class="btn btn-dark btn-outline" name="checksub"> <i class="fa fa-check"></i> Check</button>
                            ...
                            <thead><tr><th>#</th><th>CLASS</th><th>SUBJECTS NO.</th><th>DELETE</th></tr></thead>
                                <tbody> <?php  $n=0;
                                         foreach ($setclasses as $sc){  
                                            extract($sc);    $n++; ?>                                      
                        <tr><th scope="row"><?= $n; ?></th> <td><?=  $class; ?></td><td><?=  $subno; ?></td>
    <td><a href="delete.php?sbn=<?= $sn ?>" onclick="return confirm('Are you sure?')" title="Delete" ><i class="fa fa-trash fa-2x"></i></a></td>   </tr><?php } ?>
                                          </tbody>
                                    </table>
                                </div>
                        </div>
