 <?php require 'adminheader.php';  ?>          
  ...
  <button type="submit" class="btn btn-dark btn-outline" name="check"><i class="fa fa-check"></i>Check</button></div>
                            ...
                            <thead><tr><th>SN</th><th>NAME</th><th>ADMNO</th><th>SUBJECTS</th>
                             <th>AVERAGE</th><th>POSITION</th><th>PROMOTE</th><th>DETAILS</th>
                                            </tr></thead>
         
                                        <tbody><?php $n = 0;
                             foreach ($anresult as $studentresult){  $n++;  extract($studentresult);

$query->updateQuery("combr", ["pos"=> $n], ["admno"=>$admno, "class"=>$_POST['class'], "session"=>$_POST['session']]);           ?>  
      <tr style="font-weight: bold;"><td><?= $n; ?></td>
      <td class="text-primary"><?= $name; ?></td><td><?= $admno; ?></td>
      <td class="text-warning"><?= $subjtotal; ?></td><td class="text-danger"><?= round($avg, 2); ?></td>
      <td class="text-success"><?= $n; ?></td>
       <td><a href="promote.php?cid=<?= $admno; ?>&class=<?= $_POST['class']; ?>&session=<?= $_POST['session']; ?>" title="Review"><i class="fa fa-edit fa-2x"></i></a></td>
      <td><a href="cresults.php?cid=<?= $admno; ?>&class=<?= $_POST['class']; ?>&session=<?= $_POST['session']; ?>" title="Details" target="_blank"><i class="fa fa-check-circle fa-2x"></i></a></td></tr>
        <?php }  }else{ echo "SELECT TO VIEW STUDENTS' ANNUAL PERFORMANCE IN A SUBJECT"; } ?>    
                                  </tbody></table>  
                                </div>
