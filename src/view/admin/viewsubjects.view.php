...
<form action=""  method="post">
        ...
        <td>
          <?php
      if($query->lookUp("id", "termstatus", ["class"=>$gclass, "session"=>$gsession, "term"=>$gterm, "results_status"=>"approved"]) == 1){ echo "<i class='fa fa-lock'></i>"; }else{
                  ?>
          <a href="editscore.php?cid=<?= htmlentities($gadmno); ?>&class=<?= htmlentities($gclass); ?>&session=<?= $gsession; ?>&term=<?= htmlentities($gterm); ?>&subj=<?= htmlentities($subj); ?>" title="Edit Score"><i class="fa fa-edit" style="font-size:24px;"></i></a>
        <?php } ?>
        </td>
            <td><input type="checkbox" name="delete[]" id="checkbox" class="btn" value="<?= htmlentities($subj); ?>"></td>     </tr><?php }
                        if ($checkInResult < 1) {
                          ...
                          $query->updateQuery("results", ["subjectsTotal"=>$n, "gtotal"=>$totalMarks, "avg"=>$studentAvg], ["admno"=>$gadmno, "class"=>$gclass,  "session"=>$gsession, "term"=>$gterm]);  } ?> </tbody>
                          <tfoot>
                 <tr><td colspan="4"> <a href="student.php?cid=<?= htmlentities($gadmno); ?>&session=<?= $gsession; ?>" title="Back" class="btn btn-primary"><i class="fa fa-reply"></i>&nbsp;BACK</a></td>
                  <?php
                  if($query->lookUp("id", "termstatus", ["class"=>$gclass, "session"=>$gsession, "term"=>$gterm, "results_status"=>"approved"]) != 1){
                  ?>
              <td colspan="5"> <a href="addsubject.php?cid=<?= htmlentities($gadmno); ?>& class=<?= htmlentities($gclass);  ?> & session=<?php echo htmlentities($gsession); ?>& term=<?php echo htmlentities($gterm); ?>" title="Add subjects" class="btn btn-primary"><i class="fa fa-plus"></i>&nbsp;ADD</a></td>

                <td colspan="2"> <button class="btn btn-danger" name="del" type="submit" onClick="confirm('Sure want to delete this subject?')"><i class="fa fa-trash"></i>&nbsp;Delete</button></td>
              <?php } ?>
              </tr> </tfoot>    
                                                    </form>  
