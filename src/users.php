...
if (isset($_POST['submit'])) {
    ...
    if (!preg_match('/^[a-zA-Z\s]+$/', $_POST['sname'])) {
        $error['sname'] = 'Non alphabetic characters are not allowed as name';
    }

    if (!filter_var($_POST['semail'], FILTER_VALIDATE_EMAIL)) {
        $error['semail'] = 'Invalid email';
    }
    ...
    if ($_FILES['sign']['size'] == 0 && empty($_FILES['sign']['name'])) {
        ...
        if ($_FILES['sign']['size'] > 300000) {
            $error['image'] = 'Image size cannot be more than 300kb';
        }

        if (!in_array($_FILES['sign']['type'], $imgtype)) {
            $error['image'] = 'Image type not known';
        }
        $dest = $query->uploadFile('sign', 'passports/');
    }
    if ($_POST['class'] == 'admin') {
        $role = 'admin';
    } else {
        $role = 'staff';
    }
    ...
    if (empty($error)) {
        ...
        $pword = password_hash($_POST['pword'], PASSWORD_BCRYPT, ['cost' => 12]);

        $query->dbInsert('uzerz', ['ustarp' => md5($_POST['uname']), 'pvihiga' => $pword, 'name' => strtoupper($_POST['sname']), 'class' => $_POST['class'], 'signature' => $dest, 'status' => 'active', 'role' => $role, 'mobile' => $_POST['snumber'], 'em' => $_POST['semail']]);
