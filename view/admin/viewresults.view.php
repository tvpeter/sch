...
<p class="text-warning"> <?= "TOTAL SUBJECTS TO BE TAKEN BY ".$_POST['class'] ." THIS TERM IS ".$subno; ?></p>
        ...
        <p class="text-danger"> <?= "SET THE TOTAL NUMBER OF SUBJECTS TO BE TAKEN BY THIS CLASS ELSE RESULTS WILL NOT BE COMPUTED!! " ?></p>
            ...
            <thead><tr><th>SN</th><th>STUDENT NAME</th><th>ADMNO</th><th>SUBJECTS <br> TAKEN</th> <th>TOTAL <br>MARKS</th><th>AVERAGE</th><th>POS</th><th>REVIEW</th><th>PRINT</th></tr></thead>
            <tbody>  <?php $n = 0;  foreach ($resultsDetails as $rs){  $n++;  extract($rs);  ?>    
...
<td><?php if($query->lookUp("id", "termstatus", ["class"=>$_POST['class'], "session"=>$_POST['session'], "term"=>$_POST['term'], "results_status"=>"approved"]) != 1){ ?>
    <a href="resultsreview.php?cid=<?= $admno; ?>&class=<?= $_POST['class']; ?>&session=<?= $_POST['session']; ?>&term=<?= $_POST['term']; ?>" title="Review"><i class="fa fa-edit fa-2x"></i></a>
<?php } else { echo "<i class='fa fa-lock'></i>"; } ?></td>
<td><a href="results.php?cid=<?= $admno; ?>&class=<?= $_POST['class']; ?>&session=<?=  $_POST['session']; ?>&term=<?= $_POST['term']; ?>" title="Details" target="_blank"><i class="fa fa-check-circle fa-2x"></i></a></td></tr>        <?php  if ($subno !== 0) {  $avgs = $gtotal/$subno;  }else{ $avgs = 0; }
             $query->updateQuery("results", ["position"=>$n, "avg"=>$avgs], ["admno"=>$admno, "class"=>$_POST['class'], "session"=>$_POST['session'], "term"=>$_POST['term']]); 
                }}else{ echo "SELECT CLASS, SESSION AND TERM TO VIEW STUDENTS RESULTS"; } ?>    
                                    </tbody>
                                    </table>  
                                </div>
                            </div>
